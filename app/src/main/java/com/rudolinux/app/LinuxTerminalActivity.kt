...
class LinuxTerminalActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        distro = manager.available.firstOrNull { it.id == id } ?: manager.getDefault()
        ...
        val run = Button(this).apply {
            ...
            setOnClickListener {
                ...
                if (cmd.isNotBlank()) runtime.send(cmd)
                input.text.clear()
            }
        }
        ...
        runtime.start(
            ...
            onError = { error ->
                runOnUiThread {
                    output.append("\n[ERROR] ${error.message}\n")
                    Toast.makeText(this, error.message ?: "Runtime error", Toast.LENGTH_LONG).show()
                }
            }
        )
    }
