...
class RootfsManager(private val context: Context) {
    ...
    private fun extractLayer(archive: File, target: File) {
        ...
        FileInputStream(archive).use { fileInput ->
            ...
            val compressor: CompressorInputStream =
                ...
                while (entry != null) {
                    ...
                    when {
                        ...
                        current.isSymbolicLink -> {
                            ...
                            // Prefer a real Linux symlink. If Android's
                            // filesystem refuses it, copy the resolved target
                            // when it is already available in this layer.
                        ...
                        current.isLink -> {
                            ...
                            val linkTarget =
                                safeTarget(
                                    target,
                                    normalizeArchivePath(current.linkName)
                                )

                            // Materialize hard links as regular files. This
                            // avoids Android's "can't link" tar failure.
                            if (linkTarget.isFile) {
                                ...
                                // A hard-link target can occur later in the
                                // same layer. Keep a small placeholder and
                                // resolve it in a second pass.
                        ...
                        current.isFile -> {
                            ...
                            // Device nodes, FIFOs and other special entries
                            // cannot be safely created in normal app storage.
                        }
                    }
        ...
        resolvePendingHardLinks(target)
    }
    ...
    private val pendingHardLinks = mutableListOf<HardLinkPending>()

    private fun resolvePendingHardLinks(base: File) {
        if (pendingHardLinks.isEmpty()) return
        ...
        // If a broken hard-link remains, remove its empty placeholder rather
        // than leaving a fake executable in the rootfs.
        pendingHardLinks.forEach {
            if (it.destination.length() == 0L) {
                it.destination.delete()
            }
        }
        pendingHardLinks.clear()
    }
