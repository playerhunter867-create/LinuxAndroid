    ...
    private fun extractLayer(archive: File, target: File) {
        ...
        try {
            BufferedInputStream(FileInputStream(archive)).use { input ->
                // Peek at gzip magic bytes without consuming them.
                // Consuming the first two bytes before GZIPInputStream causes
                // "ZipException: Not in GZIP format" on valid gzip layers.
                input.mark(2)
                val a = input.read()
                val b = input.read()
                input.reset()
